...
| Status | 발생 조건 |
| --- | --- |
| 400 | 잘못된 요청 파라미터, 필수 파라미터 누락, 타입 불일치, 게시글 생성 검증 실패 |
| 401 | 로그인 실패, 토큰 없음, 토큰 유효하지 않음 |
| 403 | `AuthInterceptor.forbidden`에 포함된 API 요청 |
| 404 | 게시글을 찾을 수 없음 |
| 409 | 회원 ID 중복 또는 회원 ID 데이터 중복 상태 |
| 500 | 처리되지 않은 서버 오류 |
...
- 인증 쿠키가 없거나 유효하지 않으면 401 응답이 발생한다.
...
- `memberId`와 같은 회원 ID가 1개 이상 있으면 `duplicated: true`를 반환한다.
- `memberId` 요청 파라미터가 없으면 400 응답이 발생한다.
...
- 중복 회원이 있으면 409 응답이 발생한다.
- 현재 컨트롤러 응답 타입은 `Map<String, Integer>`이지만, 서비스/Mapper 흐름상 `insert()` 결과값을 반환한다.
...
- 삭제되지 않은 게시글 수를 기준으로 `CEIL(COUNT(*) / maxPostCount)` 값을 반환한다.
- `maxPostCount <= 0`이면 400 응답이 발생한다.
- `maxPostCount`가 숫자가 아니면 400 응답이 발생한다.
...
- 삭제되지 않은 게시글만 조회한다.
- 게시글 작성자의 `member_id`, `member_pk`를 함께 조회한다.
- 게시글이 없거나 삭제된 상태이면 404 응답이 발생한다.
- `post_pk`가 숫자가 아니면 400 응답이 발생한다.
...
- 삭제되지 않은 게시글만 조회한다.
- 정렬 기준은 `post_created_at DESC`, `post_pk DESC`이다.
- `offset = (page - 1) * maxPostCount`로 계산한다.
- `page <= 0` 또는 `maxPostCount <= 0`이면 400 응답이 발생한다.
- `page`, `maxPostCount`가 숫자가 아니면 400 응답이 발생한다.
...
- `postTitle`이 `null`, 빈 문자열, 금지어 포함 상태이면 400 응답이 발생한다.
- `postContent`가 `null`, 빈 문자열, 금지어 포함 상태이면 400 응답이 발생한다.
- 성공 시 생성된 게시글 PK를 반환한다.
- 인증 쿠키가 없거나 유효하지 않으면 401 응답이 발생한다.
...
- 게시글이 없거나 이미 삭제된 상태이면 404 응답이 발생한다.
- 현재 코드 기준 작성자 본인 여부 검사는 없다.
- 인증 쿠키가 없거나 유효하지 않으면 401 응답이 발생한다.
...
- 수정 전 `post_pk`가 존재하고 삭제되지 않은 게시글인지 확인한다.
- 게시글이 없거나 이미 삭제된 상태이면 404 응답이 발생한다.
...
- `Authorization` 헤더가 없으면 Spring에서 예외가 발생한다.
